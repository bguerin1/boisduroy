<?php
    if(isset($_POST["chkNote"]) && isset($_POST["btnCopie"]))
    {
        echo "kjbgrjl";
    }
?>
    ...
    <tr>
        ...
        <?php
            if($_SESSION["ADMINI"]==1)
            {
                ...
                {
                    $conn=new PDO("mysql:host=$servername;dbname=$dbname", $username,$pwd);
                    $requete3 = $conn->prepare("SELECT NOTEFRAIS.IDNOTEFRAIS,DATE_FORMAT(DATENOTEFRAIS, '%d-%m-%Y') AS DATENOTEFRAIS, NOMSTATUT, COUTTOTAL, PRENOM, NOM FROM NOTEFRAIS JOIN EMPLOYE ON EMPLOYE.MATRICULE = NOTEFRAIS.MATRICULE JOIN VALIDER ON VALIDER.IDNOTEFRAIS = NOTEFRAIS.IDNOTEFRAIS JOIN LIGNENOTE ON LIGNENOTE.IDNOTEFRAIS=NOTEFRAIS.IDNOTEFRAIS JOIN ETAPE_VALIDATION ON ETAPE_VALIDATION.IDETAPVALID = VALIDER.IDETAPVALID JOIN STATUT ON STATUT.IDSTATUT = ETAPE_VALIDATION.IDSTATUT WHERE NOMSTATUT = 'En cours de validation Responsable' AND MATRICULE_ETRE_RESPONSABLE=:matricule;");
            ...
            <div>
                ...
                <?php
                    if($_SESSION["ADMINI"]==1)
                    {
                        ...
                        {
                            if($_POST["listeDeroulantePersonne"]==0){
                                switch($_POST["listeDeroulante"])
                                {
                                    case 1:
                                        $conn=new PDO("mysql:host=$servername;dbname=$dbname", $username,$pwd);
                                        
                                        $requete3 = $conn->prepare("SELECT NOTEFRAIS.IDNOTEFRAIS,DATE_FORMAT(DATENOTEFRAIS, '%d-%m-%Y') AS DATENOTEFRAIS, NOMSTATUT, COUTTOTAL, PRENOM, NOM FROM NOTEFRAIS JOIN EMPLOYE ON EMPLOYE.MATRICULE = NOTEFRAIS.MATRICULE JOIN VALIDER ON VALIDER.IDNOTEFRAIS = NOTEFRAIS.IDNOTEFRAIS JOIN LIGNENOTE ON LIGNENOTE.IDNOTEFRAIS=NOTEFRAIS.IDNOTEFRAIS JOIN ETAPE_VALIDATION ON ETAPE_VALIDATION.IDETAPVALID = VALIDER.IDETAPVALID JOIN STATUT ON STATUT.IDSTATUT = ETAPE_VALIDATION.IDSTATUT WHERE NOTEFRAIS.MATRICULE = :matricule ORDER BY EXTRACT(YEAR FROM DATENOTEFRAIS) DESC, EXTRACT(MONTH FROM DATENOTEFRAIS) DESC, EXTRACT(DAY FROM DATENOTEFRAIS) DESC;");
                                        $requete3 ->bindValue(":matricule",$_SESSION["MATRICULE"],PDO::PARAM_STR);
                                        $requete3->execute();
                                        $data = $requete3->fetchALL(PDO::FETCH_ASSOC);
                                        $conn = null;
                                ...
                                switch($_POST["listeDeroulante"])
                                {
                                    case 1:
                                        $conn=new PDO("mysql:host=$servername;dbname=$dbname", $username,$pwd);
                                        $requete3 = $conn->prepare("SELECT NOTEFRAIS.IDNOTEFRAIS, DATE_FORMAT(DATENOTEFRAIS, '%d-%m-%Y') AS DATENOTEFRAIS, NOMSTATUT, COUTTOTAL, PRENOM, NOM FROM NOTEFRAIS JOIN EMPLOYE ON EMPLOYE.MATRICULE = NOTEFRAIS.MATRICULE JOIN VALIDER ON VALIDER.IDNOTEFRAIS = NOTEFRAIS.IDNOTEFRAIS JOIN LIGNENOTE ON LIGNENOTE.IDNOTEFRAIS=NOTEFRAIS.IDNOTEFRAIS JOIN ETAPE_VALIDATION ON ETAPE_VALIDATION.IDETAPVALID = VALIDER.IDETAPVALID JOIN STATUT ON STATUT.IDSTATUT = ETAPE_VALIDATION.IDSTATUT WHERE EMPLOYE.MATRICULE_ETRE_RESPONSABLE = :matricule ORDER BY EXTRACT(YEAR FROM DATENOTEFRAIS) DESC, EXTRACT(MONTH FROM DATENOTEFRAIS) DESC, EXTRACT(DAY FROM DATENOTEFRAIS) DESC;");
                                        $requete3 ->bindValue(":matricule",$_SESSION["MATRICULE"],PDO::PARAM_STR);
                                        $requete3->execute();
                                        $data = $requete3->fetchALL(PDO::FETCH_ASSOC);
                                        $conn = null;
                                    ...
                                    case 2:
                                        $conn=new PDO("mysql:host=$servername;dbname=$dbname", $username,$pwd);
                                        $requete3 = $conn->prepare("SELECT NOTEFRAIS.IDNOTEFRAIS, DATE_FORMAT(DATENOTEFRAIS, '%d-%m-%Y') AS DATENOTEFRAIS, NOMSTATUT, COUTTOTAL, PRENOM, NOM FROM NOTEFRAIS JOIN EMPLOYE ON EMPLOYE.MATRICULE = NOTEFRAIS.MATRICULE JOIN VALIDER ON VALIDER.IDNOTEFRAIS = NOTEFRAIS.IDNOTEFRAIS JOIN LIGNENOTE ON LIGNENOTE.IDNOTEFRAIS=NOTEFRAIS.IDNOTEFRAIS JOIN ETAPE_VALIDATION ON ETAPE_VALIDATION.IDETAPVALID = VALIDER.IDETAPVALID JOIN STATUT ON STATUT.IDSTATUT = ETAPE_VALIDATION.IDSTATUT WHERE EMPLOYE.MATRICULE_ETRE_RESPONSABLE = :matricule ORDER BY COUTTOTAL DESC;");
                                        $requete3 ->bindValue(":matricule",$_SESSION["MATRICULE"],PDO::PARAM_STR);
                                        $requete3->execute();
                                        $data = $requete3->fetchALL(PDO::FETCH_ASSOC);
                                        $conn = null;
                        ...
                        if(isset($_POST["listeDeroulante"]))
                        {
                            switch($_POST["listeDeroulante"])
                            {
                                case 1:
